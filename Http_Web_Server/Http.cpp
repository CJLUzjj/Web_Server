...
void Http::handleread()
{
	...
	std::cout << "http_fd_ is " << http_fd_ << std::endl;
	do
	{
		//int readnum = 0;
		//int readsum = 0;
...
void Http::handleConn()
{
	...
	{
		loop_->queueInLoop(std::bind(&Http::handleClose, this));//share_from_this
	}
}
...
void Http::handleClose()
{
	...
	std::shared_ptr<Channel> p(http_channel_);
	//std::cout << "http_channel_ ptr count is " << p.use_count() << std::endl;
	//std::cout << "Http ptr count is " << guard.use_count() << std::endl;
	std::cout << http_fd_ << " closing..." << std::endl;
	close(http_fd_);
}
